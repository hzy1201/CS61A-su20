...
(define (zip pairs)
  (define (helper s lst1 lst2)
    (if (null? s) (list lst1 lst2)
      (helper (cdr s) (append lst1 (cons (caar s) nil)) (append lst2 (cons (cdr (cadr s)) nil)))
    )
  )
...
(define (enumerate s)
  ...
  (define (helper num s)
    (if (null? s) nil
      (append (cons (cons num (cons (car s) nil)) nil) (helper (+ num 1) (cdr s)))
    )
  )
  (helper 0 s)
)
