...
def reverse_recursive(lst):
    ...
    t1.append(lst[n - 1])
    t2.append(lst[0])
    return t1 + reverse_recursive(lst[1 : n - 1]) + t2
...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    return sqrt((get_lat(city_a) - get_lat(city_b)) * (get_lat(city_a) - get_lat(city_b)) +
                (get_lon(city_a) - get_lon(city_b)) * (get_lon(city_a) - get_lon(city_b))
                )
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    temp = make_city('p', lat, lon)
    disa = distance(temp, city_a)
    disb = distance(temp, city_b)
    return get_name(city_a) if disa < disb else get_name(city_b)
