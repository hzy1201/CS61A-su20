def prune_min(t):
    ...
    for branch in t.branches:
        min_val = min(min_val, branch.label)
    for branch in t.branches[:]:
        if min_val != branch.label:
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            result1 = skel_char
            temp1, temp2 = helper_align(skeleton_idx + 1, code_idx + 1)
            result1 += temp1
            cost1 = temp2
            possibilities.append((result1, cost1))
        # Insert
        result2 = "+[" + code_char + "]"
        temp1, temp2 = helper_align(skeleton_idx, code_idx + 1)
        result2 += temp1
        cost2 = temp2 + 1
        possibilities.append((result2, cost2))

        # Delete
        result3 = "-[" + skel_char + "]"
        temp1, temp2 = helper_align(skeleton_idx + 1, code_idx)
        result3 += temp1
        cost3 = temp2 + 1
        possibilities.append((result3, cost3))
...
def num_splits(s, d):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(sum1, sum2, idx):
        if idx == len(s):
            if abs(sum1 - sum2) <= d:
                return 1
            return 0
        return helper(sum1 + s[idx], sum2, idx + 1) + helper(sum1, sum2 + s[idx], idx + 1)
...
def insert(link, value, index):
    ...
    IndexError
    """
    while link.rest is not Link.empty and index != 0:
        link = link.rest
        index -= 1
    if index == 0:
