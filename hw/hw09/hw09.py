...
def in_order_traversal(t):
    ...
    ans = sum([in_order_traversal(t.branches[0])], [])
    ans.append(t.label)
    ans.extend(in_order_traversal(t.branches[1]))
    return ans
...
def interleaved_sum(n, odd_term, even_term):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(temp, odd_term, even_term):
        if (temp == n + 1):
            return 0
        return helper(temp + 1, even_term, odd_term) + odd_term(temp)
