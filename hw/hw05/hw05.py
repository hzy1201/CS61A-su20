...
def make_withdraw(balance, password):
    ...
    cnt = 0
    def w(amount, passwd):
        nonlocal lst, cnt, balance
        if cnt >= 3:
            return "Too many incorrect attempts. Attempts: ['{0}', '{1}', '{2}']".format(lst[0], lst[1], lst[2])
        if password != passwd:
            cnt += 1
            lst.append(passwd)
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    if type(ans) == str:
        return ans
    def withdraw2(amount, passwd):
        ans2 = withdraw(amount, old_pass if passwd == new_pass else passwd)
        return ans2
    return withdraw2
